...
class XoT_Solver:
    def __init__(self, args, gpt, game:Game24, prompter:Game24Prompter, parser:Game24Parser, to_print=False):
        ...
        self.parser = parser
        self.nmcts, self.c = self.initial_xot(args) # 加载训练后网络，MCTS，Coach
    ...
    def multi_solve_before_revision(self, x):
        ...
        problem_state, getGameEnded, self.actions_idx, actions = self.c.generate_thoughts(x, player)
        ...
        samples = self.gpt(prompt, instruct)
    ...
    def single_solve_before_revision(self, x):
        ...
        player = lambda x: np.argmax(self.nmcts.getActionProb(x, temp=0, step=0))
        nmcts_modelcall_before = self.nmcts.getModelCall()
        problem_state, getGameEnded, self.actions_idx, actions = self.c.generate_thoughts(x, player) # 获取动作序列
        nmcts_modelcall_after = self.nmcts.getModelCall()
        model_call_phase1 = nmcts_modelcall_after - nmcts_modelcall_before
        thoughts = self.parser.action_to_thoughs(actions, x) # 包含left表达式的列表
        print('xot_solve -- 思路: ', thoughts)
        prompt, _ = self.prompter.xot_prompt_wrap(x, thoughts) # 将思路包裹在24点游戏场景的few-shot提示词中，让LLM根据思路给出
        instruct = self.prompter.get_instruction_prompt() # "用数字和基础运算符获得24."
        samples = self.gpt(prompt, instruct)
    ...
    def solve_single_revision(self, x, samples, thoughts, actions, model_call_phase1, model_call_phase2):
        ...
        for revise_count in range(self.args.xot.revise_times):
            infos = [self.parser.test_output(x, y, None) for y in samples] # [{'r': 0/1, 'revised': None}]
            isCorrect = infos[0]['r'] # 0
            # 终止条件
            if isCorrect:
            ...
            if not isCorrect:
                ...
                revised_samples = self.gpt(revised_prompt, instruct, n=1, stop='None') # 获取错误步骤
                ...
                if 'wrong' in revised_samples[0].lower() or 'incorrect' in revised_samples[0].lower():
                    try:
                        if 'all steps are wrong' in revised_samples[0].lower():
                            ...
                            incorrect_step = int(revised_samples[0].split('is wrong')[0].strip().split(']')[0][-1]) # 找到错误的一步
                        revised_state = self.parser.get_revised_state(x, thoughts, incorrect_step) # 错误步骤的前一个状态
                        if self.to_print:
                            print('incorrect_step', incorrect_step)
                        if self.args.env == 'game24':
                            if incorrect_step > 1:
                                ac_seq = self.actions_idx[:incorrect_step-1] # 获取前一个错误步骤之前的动作序列
                                state = x # [3,6,9,11]
                                for i in range(len(ac_seq)):
                                    state, _ = self.game.getNextState(state, self.actions_idx[i])
                                revised_state = state

                        nmcts_modelcall_before = self.nmcts.getModelCall()
                        player = lambda x: np.argmax(self.nmcts.getActionProb(x, temp=0, step=0))
                        problem_state, getGameEnded, self.actions_idx, actions_revised = self.c.generate_thoughts(revised_state, player)
                        nmcts_modelcall_after = self.nmcts.getModelCall()
    ...
    def solve(self, idx):
        ...
        self.c.game.total_game_step = self.args.task.total_game_step # 24点游戏场景下恒为3步
        
        if self.args.multi_solution:
            self.nmcts.args.numMCTSSims = self.args.xot.multi_numMCTSSims # 500
        else:
            self.nmcts.args.numMCTSSims = self.args.xot.numMCTSSims # 200
        ...
        if not self.args.xot.revised:
            ...
            self.c.game.total_game_step = self.args.xot.revise_total_game_step # 3
            self.nmcts.args.numMCTSSims = self.args.xot.revise_numMCTSSims # 500
